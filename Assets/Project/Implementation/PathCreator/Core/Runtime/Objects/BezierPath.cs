...
namespace PathCreation {
 ...
 IsClosed = isClosed;
        ...
        // Get latest forward
        public Vector3 LatestForward
        {
            get
            {
                return (GetAnchorPoint(NumAnchorPoints - 1) - GetAnchorPoint(NumAnchorPoints - 2)).normalized;
            }
        }
        ...
        public Vector3 GetAnchorPoint(int anchorIndex)
        {
            if (anchorIndex >= NumAnchorPoints) return Vector3.zero;

            return GetPoint(anchorIndex * 3);
        }

        public List<Vector3> GetListAnchor()
        {
