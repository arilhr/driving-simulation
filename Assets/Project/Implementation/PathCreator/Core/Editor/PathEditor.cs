    ...
    public class PathEditor : Editor {
        ...
        void DrawBezierPathInspector () {
            using (var check = new EditorGUI.ChangeCheckScope ()) {
                ...
                if (GUILayout.Button("Debug Points"))
                {
                    for (int i = 0; i < bezierPath.NumPoints; i++)
                    {
                        Debug.Log($"[DEBUG] Point {i}: {bezierPath.GetPoint(i)}");
                    }
                }

                if (GUILayout.Button("Debug Anchor Points"))
                {
                    for (int i = 0; i < bezierPath.NumAnchorPoints; i++)
                    {
                        Debug.Log($"[DEBUG] Anchor point {i}: {bezierPath.GetAnchorPoint(i)}");
                    }
                }
