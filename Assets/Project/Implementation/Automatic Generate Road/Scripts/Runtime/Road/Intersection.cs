...
namespace DrivingSimulation
{
    ...
    public class Intersection : MonoBehaviour
    {
        ...
        public void SetRoadEnd(bool right, bool forward, bool left, GameObject obj, float distance)
        {
            if (right)
                EndRoad(this.right, obj, distance);

            if (forward)
                EndRoad(this.forward, obj, distance);

            if (left)
                EndRoad(this.left, obj, distance);
        }

        private void EndRoad(PathCreator path, GameObject obj, float distance)
        {
            // spawn finish barrier
            GameObject finishBarrierObj = Instantiate(obj);
            finishBarrierObj.name = $"Finish Barrier - {gameObject.name}";
            ...
            Vector3 finishBarrierPos = path.transform.position + lastPoint1 + (lastDirection * distance);
            ...
            finishBarrierObj.transform.parent = transform;
        }
